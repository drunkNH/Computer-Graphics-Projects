...
function renderPrince() {
  ...
  antenna.normalMatrix.setInverseOf(antenna.matrix).transpose();
  antenna.render();
  ...
  antennaTop.normalMatrix.setInverseOf(antennaTop.matrix).transpose();
  antennaTop.render();
  ...
  moon.segments=10
  moon.normalMatrix.setInverseOf(moon.matrix).transpose();
  moon.renderUV();
...
function runningGuy() {
  ...
  if (g_normalOn) {
    runHead.textureNum=-3
  }
  else {
    runHead.textureNum=-2
  }
  ...
  runHead.normalMatrix.setInverseOf(runHead.matrix).transpose();
  runHead.renderUVNormal();

  runBody.matrix.setIdentity();
  if (g_normalOn) {
    runBody.textureNum=-3
  }
  else {
    runBody.textureNum=-2
  }
  ...
  runBody.normalMatrix.setInverseOf(runBody.matrix).transpose();
  runBody.renderUVNormal();

  runArm1.matrix.setIdentity();
  if (g_normalOn) {
    runArm1.textureNum=-3
  }
  else {
    runArm1.textureNum=-2
  }
  ...
  runArm1.normalMatrix.setInverseOf(runArm1.matrix).transpose();
  runArm1.renderUVNormal();

  runArm2.matrix.setIdentity();
  if (g_normalOn) {
    runArm2.textureNum=-3
  }
  else {
    runArm2.textureNum=-2
  }
  ...
  runArm2.normalMatrix.setInverseOf(runArm2.matrix).transpose();
  runArm2.renderUVNormal();

  runLeg1.matrix.setIdentity();
  if (g_normalOn) {
    runLeg1.textureNum=-3
  }
  else {
    runLeg1.textureNum=-2
  }
  ...
  runLeg1.normalMatrix.setInverseOf(runLeg1.matrix).transpose();
  runLeg1.renderUVNormal();

  runLeg2.matrix.setIdentity();
  if (g_normalOn) {
    runLeg2.textureNum=-3
  }
  else {
    runLeg2.textureNum=-2
  }
  ...
  runLeg2.normalMatrix.setInverseOf(runLeg2.matrix).transpose();
  runLeg2.renderUVNormal();
}